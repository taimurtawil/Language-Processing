...
def main():
   ...
   if grade:
      print(time.strftime('%c') + '%34s' % 'Taimur Tawil')
...
def assignmentstmt():
   advance() # advance past PRINT token 3 
   consume(ASSIGNOP)
   expr()
   

def printstmt():
    advance() # advance past PRINT token 3 consume(LEFTPAREN)
    consume(LEFTPAREN)
    expr()
    consume(RIGHTPAREN)
...
def expr():
    ...
    if(token.category == PLUS):
        while token.category == PLUS:
            advance() # no need for consume() here 5 term()
            term()
    if(token.category == MINUS):
        while token.category == MINUS:
            advance()
            term()
...
def term():
    ...
    if(token.category == TIMES):
        while token.category == TIMES:
            advance() # no need for consume() here 
            factor()
    if(token.category == DIVISION):
        while token.category == DIVISION:
            advance()
            factor()
