...
def main():
   ...
   outfile.write('@ ' + time.strftime('%c') + '%34s' % 'Taimur Tawil\n')
...
def program():
    ...
    while token.category in [NAME, PRINT]:
       outfile.write('@' + lines[token.line - 1] )
       stmt()
...
def cg_epilog():
    ...
    outfile.write('         .data\n')
    outfile.write('         .asciz "%d\\n"\n')
    
    size = len(symbol)
    i = 0
    while i < size:
        outfile.write('%-10s' % (symbol[i] + ':') + '.word' + value[i] + '\n')
        i+=1
    # missing code
  
def cg_gettemp():
    global tempcount # missing code
